...
sock.ev.on("messages.upsert", async (m) => {
  ...
  if (!msg.key.fromMe && m.type === "notify" && body === "Olá") {
    ...
    sendMessageWTyping(
      {
        text: "Oie, tudo bem? Aqui é o Pedrinho da NASA. \n👨‍🏫 Você já conhece a *COMUNIDADE ZDG?*",
      },
      msg.key.remoteJid
    );
  }
  ...
  if (!msg.key.fromMe && m.type === "notify" && body === "Sim") {
    ...
    sendMessageWTyping(
      {
        text: "✅ Então fica ligado que durante essa semana a Comunidade vai estar com um super desconto. \n\n🔔 Essa é sua última oportunidade para entrar na turma com o CUPOM DE DESCONTO: *PEDRINHODANASA*",
      },
      msg.key.remoteJid
    );
  }
...
app.post(
  ...
  async (req, res) => {
    ...
    if (numberDDD <= 30) {
      const numberZDG =
        numberDDI + numberDDD + "9" + numberUser + "@s.whatsapp.net";
      await sendMessageWTyping(message, numberZDG)
        .then((response) => {
          res.status(200).json({
            status: true,
